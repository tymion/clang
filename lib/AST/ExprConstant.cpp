...
/// field or base class.
static APValue::BaseOrMemberType getAsBaseOrMember(APValue::LValuePathEntry E) {
...
public:
  ...
  void endLifetime() { *Value.getPointer() = APValue(); }
};
...
struct EvalInfo {
  ...
  struct EvaluatingConstructorRAII {
    ...
    ~EvaluatingConstructorRAII() {
      if (DidInsert)
        EI.EvaluatingConstructors.erase(Object);
    }
  };
...
public:
  ...
  void addNotes(ArrayRef<PartialDiagnosticAt> Diags) {
    if (HasActiveDiagnostic) {
      EvalStatus.Diag->insert(EvalStatus.Diag->end(), Diags.begin(),
                              Diags.end());
    }
  }
...
template <bool IsFullExpression> class ScopeRAII {
  EvalInfo &Info;
...
private:
  static void cleanup(EvalInfo &Info, unsigned OldStackSize) {
    ...
    for (unsigned I = OldStackSize, N = Info.CleanupStack.size(); I != N; ++I) {
...
static bool handleFloatFloatBinOp(EvalInfo &Info, const Expr *E, APFloat &LHS,
...
static bool evaluateVarDeclInit(EvalInfo &Info, const Expr *E,
  ...
  if (!VD->evaluateValue(Notes)) {
    Info.FFDiag(E, diag::note_constexpr_var_init_non_constant, Notes.size() + 1)
        << VD;
    Info.Note(VD->getLocation(), diag::note_declared_at);
    Info.addNotes(Notes);
    return false;
...
findSubobject(EvalInfo &Info, const Expr *E, const CompleteObject &Obj,
  ...
  for (unsigned I = 0, N = Sub.Entries.size(); /**/; ++I) {
    ...
    if (I == N) {
      ...
      if (handler.AccessKind != AK_Read && LastField &&
    ...
    if (ObjType->isArrayType()) {
      ...
      if (O->isLValue()) {
        ...
        if (handler.AccessKind != AK_Read)
          expandStringLiteral(Info, O->getLValueBase().get<const Expr *>(), *O);
        else
...
static CompleteObject findCompleteObject(EvalInfo &Info, const Expr *E,
  ...
  if (const ValueDecl *D = LVal.Base.dyn_cast<const ValueDecl *>()) {
    ...
    if (BaseType.isVolatileQualified()) {
      if (Info.getLangOpts().CPlusPlus) {
        Info.FFDiag(E, diag::note_constexpr_access_volatile_obj, 1) << AK << 0;
...
static bool handleCompoundAssignment(EvalInfo &Info, const Expr *E,
...
static EvalStmtResult EvaluateSwitch(StmtResult &Result, EvalInfo &Info,
  ...
  switch (EvalStmtResult ESR =
              EvaluateStmt(Result, Info, SS->getBody(), Found)) {
  case ESR_Break:
    return ESR_Succeeded;
  case ESR_Succeeded:
  case ESR_Continue:
...
static bool HandleConstructorCall(const Expr *E, const LValue &This,
  ...
  return HandleConstructorCall(E, This, ArgValues.data(), Definition, Info,
                               Result);
}
...
public:
  ...
  bool handleCallExpr(const CallExpr *E, APValue &Result,
    ...
    if (This && !HasQualifier && isa<CXXMethodDecl>(FD) &&
...
bool PointerExprEvaluator::VisitCastExpr(const CastExpr *E) {
  ...
  case CK_ArrayToPointerDecay: {
    if (SubExpr->isGLValue()) {
      ...
      APValue &Value =
          createTemporary(SubExpr, false, Result, *Info.CurrentCall);
...
bool IntExprEvaluator::VisitBuiltinCallExpr(const CallExpr *E,
  ...
  case Builtin::BI__builtin_wmemcmp: {
    ...
    bool StopAtNull =
        (BuiltinOp != Builtin::BImemcmp && BuiltinOp != Builtin::BIwmemcmp &&
         BuiltinOp != Builtin::BI__builtin_memcmp &&
...
bool IntExprEvaluator::VisitBinaryOperator(const BinaryOperator *E) {
  ...
  if (E->isComparisonOp()) {
    ...
    auto OnSuccess = [&](ComparisonCategoryResult ResKind,
                         const BinaryOperator *E) {
      using CCR = ComparisonCategoryResult;
      bool IsEqual = ResKind == CCR::Equal, IsLess = ResKind == CCR::Less,
...
bool ComplexExprEvaluator::VisitCastExpr(const CastExpr *E) {
  ...
  case CK_IntegralComplexToFloatingComplex: {
    ...
    Result.makeComplexFloat();
    return HandleIntToFloatCast(Info, E, From, Result.IntReal, To,
                                Result.FloatReal) &&
           HandleIntToFloatCast(Info, E, From, Result.IntImag, To,
                                Result.FloatImag);
  }
  }
...
bool Expr::EvaluateAsInitializer(
    APValue &Value, const ASTContext &Ctx, const VarDecl *VD,
  ...
  EvalInfo InitInfo(Ctx, EStatus,
                    VD->isConstexpr() ? EvalInfo::EM_ConstantExpression
...
#include "clang/AST/StmtNodes.inc"
  ...
  case Expr::DeclRefExprClass: {
    ...
    if (Ctx.getLangOpts().CPlusPlus && D && IsConstNonVolatile(D->getType())) {
...
bool Expr::isMetaobjectIdExpr(llvm::APSInt &Value, void *InfoPtr,
