...
bool Parser::ParseUsingDeclarator(DeclaratorContext Context,
  ...
  if (getLangOpts().CPlusPlus11 &&
      Context == DeclaratorContext::MemberContext && Tok.is(tok::identifier) &&
    ...
    if (ParseUnqualifiedId(
            ...
            /*AllowDeductionGuide=*/false, nullptr, nullptr, D.Name))
      return true;
  }
...
SourceLocation Parser::ParseUnrefltypeSpecifier(DeclSpec &DS) {
  ...
  if (Tok.is(tok::annot___unrefltype)) {
    ...
    EnterUnrefltypeEvaluationContext EUTEval(
        Actions, Sema::ExpressionEvaluationContext::ConstantEvaluated);
  ...
  if (TemplateId) {
    ...
    if (TemplateInfo.Kind == ParsedTemplateInfo::ExplicitInstantiation &&
      ...
      TypeResult = Actions.ActOnTagTemplateIdType(
          TUK, TagType, StartLoc, TemplateId->SS, TemplateId->TemplateKWLoc,
...
MemInitResult Parser::ParseMemInitializer(Decl *ConstructorDecl) {
  ...
  if (getLangOpts().CPlusPlus11 && Tok.is(tok::l_brace)) {
    ...
    return Actions.ActOnMemInitializer(
        ConstructorDecl, getCurScope(), SS, II, TemplateTypeTy, DS, IdLoc,
